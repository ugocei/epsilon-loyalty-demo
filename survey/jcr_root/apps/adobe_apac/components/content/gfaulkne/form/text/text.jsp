        ...
        <div class="form_rightcol" id="<%= name %>_rightcol"><%
            ...
            for (String value : values) {
                ...
                if (readOnly) {
                    ...
                    if (rows.equals("1")) {
                        %><input class="<%= FormsHelper.getCss(properties, "form_field form_field_text" + (multiValued ? " form_field_multivalued" : "" )) %>" id="<%= currentId %>" name="<%= name %>" size="<%= cols %>" <%= (width != null ? "style='width:" + width + "px;'" : "") %>><%
                    } else {
                    ...
                    String defaultCloudValue = properties.get("defaultCloudValue", null);
                    if (defaultCloudValue != null ) { %>
<script>
try {
    var thisTextField = document.getElementById('<%= currentId %>');             
    var cloudValue =  CQ_Analytics.ProfileDataMgr.getProperty('<%= defaultCloudValue %>');
    if (cloudValue != null && cloudValue != 'undefined') {
        thisTextField.value = "" + cloudValue;
    }
} catch(e) {
}
</script> <% 
